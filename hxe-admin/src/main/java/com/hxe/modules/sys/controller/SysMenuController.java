...
import com.hxe.core.sys.entity.SysMenuEntity;
import com.hxe.core.sys.service.SysMenuService;
...
public class SysMenuController extends AbstractController {
	...
	public Results nav(){
		...
		return Results.ok().put("menuList", menuList);
	}
	...
	public Results info(@PathVariable("menuId") Long menuId){
		SysMenuEntity menu = sysMenuService.selectById(menuId);
		return Results.ok().put("menu", menu);
	}
...
//	@SysLog("保存菜单")
	...
	public Results save(@RequestBody SysMenuEntity menu){
		//数据校验
		verifyForm(menu);
...
//	@SysLog("修改菜单")
	...
	public Results update(@RequestBody SysMenuEntity menu){
		//数据校验
		verifyForm(menu);
...
//	@SysLog("删除菜单")
	...
	public Results delete(long menuId){
		if(menuId <= 31){
			return Results.error("系统菜单，不能删除");
		}

		//判断是否有子菜单或按钮
		List<SysMenuEntity> menuList = sysMenuService.queryListParentId(menuId);
		if(menuList.size() > 0){
			return Results.error("请先删除子菜单或按钮");
		}
